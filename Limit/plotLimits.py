...
mxx = args.masspoint.split('_')[0].split('-')[-1]
mzd = args.masspoint.split('_')[1].split('-')[-1].replace('p', '.')

setattr(args, 'x_title', 'c#tau [mm]')
setattr(args, 'y_title', '95% CL upper limit on #sigma(pp#rightarrow#chi#bar{#chi}#rightarrowZ_{d}Z_{d}) [pb]')
setattr(args, 'title_left', "m_{#chi#bar{#chi}}, m_{Z_{d}} (%s, %s) GeV" % (mxx, mzd))
setattr(args, 'title_right', "59.74 fb^{-1} (13 TeV)")
setattr(args, 'cms_sub', 'Work-in-progress')
setattr(args, 'logy', True)
setattr(args, 'logx', True)
# if args.withsyst:
#     args.input = ['datacards/%s/%s/limits.json' % (args.channel, args.masspoint),]
#     args.output = 'limitplots/%s/%s' % (args.channel, args.masspoint)
# else:
#     args.input = ['datacards.nosyst/%s/%s/limits.json' % (args.channel, args.masspoint),]
#     args.output = 'limitplots.nosyst/%s/%s' % (args.channel, args.masspoint)
