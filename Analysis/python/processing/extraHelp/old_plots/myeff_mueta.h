...
void myeff_mueta(int date, const char *muon)
//void mueta(int num)
{
	//char C[10] = "dsa";
    ...
    mueta_s1->SetStats(0);
    mueta_s1->SetTitle(Form("Sub-leading #eta distribution: %s muons",muon));
    ...
    leg_mueta->Draw();    
    can_mueta->SaveAs(Form("../outputs/plots/modules/TrEff2mu/%i_%s_mueta.pdf",date,muon));
    
    //all mu eta for != lxy
    TCanvas* can_muetalxy = new TCanvas("can_muetalxy"); 
    mueta_s4->SetStats(0);    
    mueta_s4->SetTitle(Form("Sub-leading #eta distribution: %s muons",muon));
    ...
    leg_muetalxy->Draw();    
    can_muetalxy->SaveAs(Form("../outputs/plots/modules/TrEff2mu/%i_%s_muetalxy.pdf",date,muon));	
//    cout<<"done with you"<<endl;
}
