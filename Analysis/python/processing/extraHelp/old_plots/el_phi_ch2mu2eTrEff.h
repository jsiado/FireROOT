void el_phi_ch2mu2eTrEff(const char *date, const char *filter)
{
	//TFile* file_1 = TFile::Open(Form("../outputs/rootfiles/modules/el_ch2mu2eTrEff_%s.root",filter));
    ...
    //num all samples
	TH1F *num_s1 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_phi",S1));	TH1F *eff_s1 = (TH1F*) num_s1->Clone();
	TH1F *num_s3 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_phi",S3));	TH1F *eff_s3 = (TH1F*) num_s3->Clone();
	TH1F *num_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_phi",S6));	TH1F *eff_s6 = (TH1F*) num_s6->Clone();
	

    //den all samples
	TH1F *den_s1 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_phi",S1));        
	TH1F *den_s3 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_phi",S3));
	TH1F *den_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_phi",S6));
	...
	TCanvas *can_phi = new TCanvas("can_phi","",800,600);
	eff_s1->SetTitle("Leaading muon #Phi; #Delta R; Efficiency");
	...
	leg_f1->Draw();    
	can_f1->SaveAs(Form("../outputs/plots/modules/ch2mu2eTrEff/elte_%s_dR_%s.pdf",date,filter));
	can_f1->SaveAs(Form("../outputs/plots/modules/ch2mu2eTrEff/elte_%s_dR_%s.png",date,filter));
}
