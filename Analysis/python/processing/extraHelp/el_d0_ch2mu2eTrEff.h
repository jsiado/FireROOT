void el_d0_ch2mu2eTrEff(const char *date, const char *filter, const char *muon)
{
	...
	TH1F *num_s5 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_d0",S5));	TH1F *eff_s5 = (TH1F*) num_s5->Clone();
	TH1F *num_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_d0",S6));	TH1F *eff_s6 = (TH1F*) num_s6->Clone();
	...
	TH1F *den_s5 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_d0",S5));
	TH1F *den_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_d0",S6));
	...
	eff_s1->SetTitle(Form("Leading %s muon d_{0}; d_{0} [cm]; Efficiency",muon));
	...
	leg_d0->AddEntry(eff_s5, Form("%s",S5), "P");
	leg_d0->AddEntry(eff_s6, Form("%s",S6), "P");    
	leg_d0->Draw();    
	can_d0->SaveAs(Form("../outputs/plots/modules/el_ch2mu2eTrEff/elte_%s_d0_%s_%s.pdf",date,filter,muon));
	can_d0->SaveAs(Form("../outputs/plots/modules/el_ch2mu2eTrEff/elte_%s_d0_%s_%s.png",date,filter,muon));
}
