void ch2mu2eTrEff_zdlxy(const char *np, const char *rt, const char *ch){
    ...
    TCanvas *canx = new TCanvas(var, var, 800, 600);
    TLegend *legx = new TLegend(.50, .72, .88, .88);

    legx->SetHeader("Samples:","C");
	...
	axis->SetLimits(0.,800.3);
    	gPad->Update();
	trEff->SetTitle(" ; Dark phton L_{xy} [cm]; Efficiency (#epsilon)");
	legx->AddEntry(trEff, Form("%s",S1), "l");
    }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	legx->AddEntry(trEff, Form("%s",S2), "l");
      }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	legx->AddEntry(trEff, Form("%s",S3), "l");	
      }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	legx->AddEntry(trEff, Form("%s",S4), "l");
      }
    legx->Draw();
    canx->SaveAs(Form("extraHelp/plots/%s_%s_TrEff_%s.pdf",np, ch, var));
}
