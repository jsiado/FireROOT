void ch2mu2eTrEff_pTlxyl(const char *np, const char *rt, const char *ch){
    ...
    TCanvas *can = new TCanvas(var, var, 800, 600);
    TLegend *leg = new TLegend(.50, .72, .88, .88);
	...
	axis->SetLimits(0.,600.);
    	gPad->Update();
	trEff->SetTitle(" 75 < Z_{d}^{L_{xy}} < 400 cm; p_{T}^{gen} [GeV] ; Efficiency (#epsilon)");
	leg->AddEntry(trEff, Form("%s",S1), "l");
    }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	leg->AddEntry(trEff, Form("%s",S2), "l");
      }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	leg->AddEntry(trEff, Form("%s",S3), "l");	
      }
	...
	auto graph = trEff->GetPaintedGraph();
	gPad->Update();
	leg->AddEntry(trEff, Form("%s",S4), "l");
    }
    leg->Draw();
    can->SaveAs(Form("extraHelp/plots/%s_%s_TrEff_%s.pdf",np, ch, var));
}
