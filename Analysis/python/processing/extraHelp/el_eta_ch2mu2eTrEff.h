void el_eta_ch2mu2eTrEff(const char *date, const char *filter, const char *muon)
{	
	...
	TH1F *num_s5 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_eta",S5));	TH1F *eff_s5 = (TH1F*) num_s5->Clone();
	TH1F *num_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Num_eta",S6));	TH1F *eff_s6 = (TH1F*) num_s6->Clone();
	...
	TH1F *den_s5 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_eta",S5));
	TH1F *den_s6 = (TH1F*)file_1->Get(Form("ch2mu2e/sig/%s/TO_Den_eta",S6));
	...
	eff_s1->SetTitle(Form("Leading %s muon #eta; #eta; Efficiency",muon));
	...
	leg_f1->Draw();
	can_eta->SaveAs(Form("../outputs/plots/modules/el_ch2mu2eTrEff/elte_%s_eta_%s_%s.pdf",date,filter,muon));
	can_eta->SaveAs(Form("../outputs/plots/modules/el_ch2mu2eTrEff/elte_%s_eta_%s_%s.png",date,filter,muon));
}
