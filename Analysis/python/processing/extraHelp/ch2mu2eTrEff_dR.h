void ch2mu2eTrEff_dR(const char *np, const char *rt, const char *ch){
	...
	axis->SetLimits(0.,.3);
    	gPad->Update();
	trEff->SetTitle(" ; #Delta R_{gen}^{lea} ; Efficiency (#epsilon)");
    ...
    leg->Draw();
    can->SaveAs(Form("extraHelp/plots/%s_%s_TrEff_%s.png",np,ch, var));
}
