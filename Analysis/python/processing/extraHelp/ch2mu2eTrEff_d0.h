void ch2mu2eTrEff_d0(const char *np, const char *rt, const char *ch){
    ...
    TCanvas *can0 = new TCanvas("can_d0","can_d0",800,600);
	...
	axis->SetLimits(0.,100.);
    	gPad->Update();
	trEff->SetTitle(" ; d_{0} [cm] ; Efficiency (#epsilon)");
    ...
    legr->Draw();
    can0->SaveAs(Form("extraHelp/plots/%s_%s_TrEff_%s.pdf",np, ch, var));
}
