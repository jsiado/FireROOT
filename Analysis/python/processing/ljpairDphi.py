...
class MyEvents(Events):
    ...
    def processEvent(self, event, aux):
        ...
        passMuD0 = all(map(lambda lj: not (lj.isMuonType() and lj.minTkD0()>0 and lj.minTkD0()<=0.1), [LJ0, LJ1]))
        ...
        if not (passCosmic and passIso and passFake): return

        if self.Type=='DATA' and abs(DeltaPhi(LJ0.p4, LJ1.p4))>ROOT.Math.Pi()/2: return
...
for ds, files in bkgDS.items():
    ...
    for chan in ['2mu2e', '4mu']:
        events_.bookHisto('dphi-{}'.format(chan), ROOT.Hist(20,0, ROOT.Math.Pi(),
...
for ds in sampleSig:
    ...
    events_.bookHisto('dphi-4mu', ROOT.Hist(20,0, ROOT.Math.Pi(),
...
for ds in sampleSig:
    ...
    events_.bookHisto('dphi-2mu2e', ROOT.Hist(20,0, ROOT.Math.Pi(),
                                        title=ds, drawstyle='hist pmc plc', legendstyle='L', linewidth=2))
    events_.process()
...
for chan in ['2mu2e', '4mu']:
    ...
    legend = ROOT.Legend(hstack.GetHists()+[stackError]+sigHistList+[DataHists[histName]],
                        pad=c, leftmargin=0.05, margin=0.1, entryheight=0.02, textsize=12)
    # legend.SetNColumns(2)
    legend.Draw()
    title = TitleAsLatex("[{}] lepton-jet pair #Delta#phi".format(channelLatex))
    title.Draw()

    c.Print('ljpairDphi-{}.pdf'.format(chan))
