...
Eff = gen mathed reco muons matched to TO / gen muons.
'''
...
class MyEvents(SignalEvents):
    ...
    def processEvent(self, event, aux):
        ...
        if m1 is not None: #a reco muon was matched to the leading gen muon
        ...
        for rr, rrmu in enumerate(reco_mu):
            ...
            ptr = rrmu.p4.pt()
            if dr < min2 and rr != m1 and  ptr > 0.000001*ptg2 and ptr < 99999.9*ptg2: #adjust numbers if pt matching will be used.
                min2 = dr
                if min2< drthr_gm:
...
histCollection = [
    ...
    { 'name': 'diff1_pT',         'binning': (100, -10,10),     'title': 'p_{T} diff of leading muons; #frac{p_{T}^{gen} - p_{T}^{reco}}{p_{T}^{gen}} [GeV]; Events'},
    ...
    { 'name': 'diff2_pT',         'binning': (100, -10,10),     'title':'p_{T} diff of sub-leading muons; #frac{p_{T}^{gen} - p_{T}^{reco}}{p_{T}^{gen}} [GeV]; Events'},
