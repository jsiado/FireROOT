...
class MyEvents(Events):
    ...
    def processEvent(self, event, aux):
        ...
        if len(Reco_Mu) == 2:#Reco_Mu[0] is the leading muon
            ...
            self.Histos['%s/ReMu_d0' % chan].Fill(d0_1)
            self.Histos['%s/ReMu_d0' % chan].Fill(d0_2)
            ...
            #find the min delta R between reco mu "0" and its closest TO
            mindr,to1 = 999,999
            for k, TO in enumerate(TO_pass):
                ...
                if dr_to1mu1 < mindr:
                    ...
                    if mindr < dR_thr:#matched
                        ...
                        mud0 = abs(Reco_Mu[mu1].d0)*10000
                        dr11 = mindr #dr(to1,mu1)
 
            if to1 != 999:#store variables related to muon 1
                ...
                if mud0 < 200: #still does not work properly
...
histCollection = [
    { 'name': 'dsaMu_d0',           'binning': (50, 0, 2500),          'title': '|d_{0}| dsa muons; |d_{0}|[#mu m]; Events'},
    ...
    { 'name': 'pfMu_d0',            'binning': (50, 0, 2500),          'title': '|d_{0}| pf muons; |d_{0}|[#mu m]; Entries'},
    ...
    { 'name': 'ReMu_d0',            'binning': (50, 0, 2500),          'title': '|d_{0}| reco muons; |d_{0}|[#mu m]; Entries'},
    ...
    { 'name': 'TO_Num_dR',          'binning': (16, 0.0, 0.4),         'title': '#Delta R matched; #Delta R(#mu_{1},#mu_{2}); Entries'},
    { 'name': 'TO_Den_dR',          'binning': (16, 0.0, 0.4),         'title': '#Delta R total; #Delta R(#mu_{1},#mu_{2}); Entries'},
    { 'name': 'TO_Num_pT',          'binning': [[0,5,10,15,20,25,30,35,40,50,60,70,80,90,100,120,140,160,200,240,300,400,500,600,]],         'title': 'p_{T} matched; p_{T} [GeV]; Entries'},
    { 'name': 'TO_Den_pT',          'binning': [[0,5,10,15,20,25,30,35,40,50,60,70,80,90,100,120,140,160,200,240,300,400,500,600,]],         'title': 'p_{T} total; p_{T} [GeV]; Entries'},
