...
fn = os.path.join(os.getenv('CMSSW_BASE'), 'src/FireROOT/Analysis/python/outputs/rootfiles/leptonjetCosmicVeto.root')
outdir = os.path.join(os.getenv('CMSSW_BASE'), 'src/FireROOT/Analysis/python/etc/plots/leptonjetCosmicVetoCutEfficiency')
...
def routine(chan, varname):
    ...
    for t in chandir.sig.keys():
        ...
        for i in range(1, h.nbins()+1):
            ...
            h_[i].error = 0
            if i==5: effs[sigtag] = h.integral(xbin1=i, overflow=True)/h_total
        h_.title = sigtag
    ...
    if h_total!=0:
        ...
        for i in range(1, h.nbins()+1):
            h_[i] = h.integral(xbin1=i, overflow=True)/h_total
            h_[i].error = 0
            if i==5: effs['data'] = h.integral(xbin1=i, overflow=True)/h_total
        h_.drawstyle = 'hist'
        h_.title='data'
        h_.color = 'black'
        h_.linestyle = 'dashed'
    ...
    axes, limits =draw(hs, ylimits=(0., 1.05), ytitle='(forward) cut efficiency',)
    legend.Draw()
    title = TitleAsLatex('[{}] {} cut efficiency'.format(chan.replace('mu', '#mu'), varname))
    title.Draw()
